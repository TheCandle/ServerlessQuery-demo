...
// ConnPack encapsulates the information needed to connect remote stream threads
//
typedef struct ConnPack ConnPack;
...
typedef struct pgNotify {
    ...
    ThreadId be_pid; /* process ID of notifying server process */
...
typedef struct _PQconninfoOption {
    ...
    size_t valsize; /* Value actual length for double check */
} PQconninfoOption;
...
extern char* PQbuildPGconn(const char* conninfo, PGconn** connPtr, int* packetlen);
...
extern int PQcancel_timeout(PGcancel* cancel, char* errbuf, int errbufsize, int timeout);

/* issue a stop request */
extern int PQStop(PGcancel* cancel, char* errbuf, int errbufsize, unsigned long query_id);
extern int PQstop_timeout(PGcancel* cancel, char* errbuf, int errbufsize, int timeout, unsigned long query_id);

/* issue a stream connect request */
extern int StreamConnect(void* raddr, ConnPack* csp, int packetlen, char* errbuf, int errbufsize);
...
extern struct sockaddr* PQLocalSockaddr(const PGconn* conn);
extern struct sockaddr* PQRemoteSockaddr(const PGconn* conn);
...
extern int PQsetRwTimeout(PGconn* conn, int build_receive_timeout);
...
extern long int lo_lseek64(PGconn *conn, int fd, long int offset, int whence);
extern Oid lo_creat(PGconn* conn, int mode);
extern Oid lo_create(PGconn* conn, Oid lobjId);
extern int lo_tell(PGconn* conn, int fd);
extern long int lo_tell64(PGconn *conn, int fd);
extern int lo_truncate(PGconn* conn, int fd, size_t len);
extern int lo_truncate64(PGconn *conn, int fd, long int len);
...
extern void parseInput(PGconn* conn);

extern PGresult* getCopyResult(PGconn* conn, ExecStatusType copytype);
