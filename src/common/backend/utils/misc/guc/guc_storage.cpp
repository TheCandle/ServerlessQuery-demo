...
static void InitStorageConfigureNamesBool()
{
    struct config_bool localConfigureNamesBool[] = {
        ...
        {{"enable_pq",
            ...
            NODE_SINGLENODE,
            QUERY_TUNING_OTHER,
            gettext_noop("Whether enable pq in datavec"),
            NULL},
            &g_instance.attr.attr_storage.enable_pq,
            false,
            NULL,
            NULL,
            NULL},
...
static bool check_ss_fi_cpu_latency_entries(char** newval, void** extra, GucSource source)
{
    ...
    if (ss_fi_set_entries(DDES_FI_TYPE_NET_LATENCY, entry_list, count) != DMS_SUCCESS) {
        ereport(WARNING, (errmsg("[DMS]set parameter ss_fi_cpu_latency_entries fail")));
        return false;
    }
    return true;
}
